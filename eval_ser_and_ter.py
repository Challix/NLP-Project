...
def eval_ser_and_ter(dataset, text_model, audio_model):
    data_loader = data.DataLoader(dataset,
                                  batch_size=64,
                                  shuffle=False,
                                  num_workers=4,
...
test_audio = processor(test_waves, sampling_rate=16000, padding=True, truncation=True, max_length=64000)

test_dataset = SerAndTerDataset(test_encodings, test_audio, test_labels)

num_labels = len(test_dataset_.targets)

text_model = DistilBertForSequenceClassification.from_pretrained("results_ter/checkpoint-184", local_files_only=True, output_hidden_states=True)
audio_model = Wav2Vec2ForSequenceClassification.from_pretrained("results_ser/checkpoint-14605", local_files_only=True, output_hidden_states=True)

# print('test')
# eval_ter(test_dataset_text, text_model)
# eval_ser(test_dataset_audio, audio_model)

res = eval_ser_and_ter(test_dataset, text_model, audio_model)
print(res)
